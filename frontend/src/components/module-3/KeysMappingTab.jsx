...
const KeysMappingTab = ({ file1State, file2State, mappingState, setMappingState, onNext }) => {
    const { log } = useConsole();

    const sourceColumns = file1State.columns;
    const targetColumns = file2State.columns;

    useEffect(() => {
        if (mappingState.initialized) return;
        const initial = {};
        sourceColumns.forEach(col => {
            const match = targetColumns.find(tc => tc.toLowerCase() === col.toLowerCase());
            initial[col] = match || '';
        });
        setMappingState({ mapping: initial, keys: [], initialized: true });
        log(`Auto-mapped ${Object.values(initial).filter(Boolean).length} / ${sourceColumns.length} columns by name match`, 'info');
    ...
    const handleMappingChange = (srcCol, tgtCol) => {
        const next = { ...mapping, [srcCol]: tgtCol };
        setMapping(next);
        if (!tgtCol && selectedKeys.includes(srcCol)) setSelectedKeys(selectedKeys.filter(k => k !== srcCol));
        if (tgtCol) log(`Mapped: ${srcCol} → ${tgtCol}`, 'info'); else log(`Unmapped: ${srcCol}`, 'warn');
    };

    const toggleKey = (col) => {
        if (!mapping[col]) return;
        if (selectedKeys.includes(col)) { setSelectedKeys(selectedKeys.filter(k => k !== col)); log(`Removed key: ${col}`, 'warn'); }
        else { setSelectedKeys([...selectedKeys, col]); log(`Added key: ${col}`, 'success'); }
    };

    const mappedPairs = useMemo(() => Object.entries(mapping).filter(([, v]) => v !== ''), [mapping]);
    const unmappedCount = sourceColumns.length - mappedPairs.length;
    const usedTargetColumns = useMemo(() => Object.values(mapping).filter(Boolean), [mapping]);
                ...
                <table className="w-full text-xs">
                    <thead className="bg-slate-100 sticky top-0 z-10">
                        <tr>
                            ...
                            <th className="px-3 py-2 text-left text-brand-900 font-bold">File 1 Column</th>
                            <th className="px-3 py-2 text-center w-8"><ArrowRight className="w-3 h-3 mx-auto text-gray-400" /></th>
                            <th className="px-3 py-2 text-left text-brand-500 font-bold">File 2 Column</th>
                            <th className="px-3 py-2 text-center w-16">Status</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {sourceColumns.map(srcCol => {
                            const isMapped = mapping[srcCol] && mapping[srcCol] !== '';
                            const isKey = selectedKeys.includes(srcCol);
                            return (
                                <tr key={srcCol} className={`${isMapped ? 'bg-white' : 'bg-amber-50/50'} hover:bg-slate-50 transition-colors`}>
                                    <td className="px-3 py-2 text-center">
                                        <button onClick={() => toggleKey(srcCol)} disabled={!isMapped}
                                    ...
                                    <td className="px-3 py-2 font-mono font-bold text-brand-900">{srcCol}</td>
                                    <td className="px-3 py-2 text-center"><ArrowRight className={`w-3 h-3 mx-auto ${isMapped ? 'text-brand-500' : 'text-gray-300'}`} /></td>
                                    <td className="px-3 py-2">
                                        <select value={mapping[srcCol] || ''} onChange={e => handleMappingChange(srcCol, e.target.value)}
                                            ...
                                            {targetColumns.map(tc => <option key={tc} value={tc} disabled={usedTargetColumns.includes(tc) && mapping[srcCol] !== tc}>{tc}{usedTargetColumns.includes(tc) && mapping[srcCol] !== tc ? ' (used)' : ''}</option>)}
                                        </select>
                                    </td>
                                    <td className="px-3 py-2 text-center">
