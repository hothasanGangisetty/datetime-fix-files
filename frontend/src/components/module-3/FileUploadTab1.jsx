...
const FileUploadTab1 = ({ fileState, setFileState, onNext }) => {
    ...
    const handleFile = async (file) => {
        ...
        setLoading(true);
        log(`[File 1] Uploading: ${file.name} (${(file.size / 1024).toFixed(1)} KB)...`, 'info');
        ...
        try {
            ...
            log(`[File 1] Uploaded: ${file.name} — ${total_rows} rows, ${(columns || []).length} columns`, 'success');
            logTable(columns || [], preview_data || [], 10);
            log('→ Click Next to upload File 2', 'system');
        } catch (err) { log(`[File 1] Upload failed: ${err.response?.data?.message || err.message}`, 'error'); }
        finally { setLoading(false); }
    };

    const handleRemove = () => {
        log(`[File 1] Removed: ${fileState.fileName}`, 'warn');
    ...
    return (
        <div className="h-full flex flex-col p-4 gap-4">
            <div className="flex items-center gap-2 text-slate-700">
                ...
                <h3 className="font-bold text-sm">File Upload — Source 1</h3>
            ...
            {!fileState.uploaded ? (
                <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
                    ...
                    {loading ? (
                        ...
                        <><Upload className="w-10 h-10 text-gray-400" /><span className="text-sm text-gray-500 font-medium">Drag & drop or click — File 1</span><span className="text-xs text-gray-400">Supports .csv, .xlsx, .xls</span></>
                    )}
                </div>
            ) : (
