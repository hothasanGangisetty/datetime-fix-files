...
const SqlQueryTab1 = ({ connection, sqlState, setSqlState, onNext }) => {
    ...
    const handleExecute = async () => {
        ...
        setLoading(true);
        log(`[SQL 1] Executing query on ${connection.database}...`, 'info');
        ...
        try {
            ...
            log(`[SQL 1] Query executed — ${row_count_estimate} rows fetched`, 'success');
            log(`Columns: ${(columns || []).join(', ')}`, 'info');
            logTable(columns || [], preview_data || [], 5, { lastRow: res.data.last_row, totalRows: row_count_estimate });
            log('→ Click Next to proceed to SQL Query 2', 'system');
        } catch (err) {
            log(`[SQL 1] Query failed: ${err.response?.data?.message || err.message}`, 'error');
    ...
    return (
        <div className="h-full flex flex-col p-4 gap-4">
            <div className="flex items-center gap-2 text-slate-700">
                ...
                <h3 className="font-bold text-sm">SQL Query — Source 1</h3>
            ...
            <textarea className="flex-1 w-full p-3 border border-gray-300 rounded-lg font-mono text-sm bg-slate-50 focus:border-brand-700 focus:ring-1 focus:ring-brand-700 outline-none resize-none min-h-[120px]"
                ...
                placeholder="SELECT * FROM SourceTable1" spellCheck={false} />
            <div className="flex justify-between items-center">
                <span className="text-xs text-gray-400">{sqlState.executed ? `${sqlState.columns.length} columns, ~${sqlState.count} rows` : 'Write a SELECT query for Source 1'}</span>
