...
const FileUploadTab = ({ fileState, setFileState, onNext }) => {
    ...
    const handleFile = async (file) => {
        ...
        } catch (err) { log(`Upload failed: ${err.response?.data?.message || err.message}`, 'error'); }
            ...
            {!fileState.uploaded ? (
                <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
                    ...
                    {loading ? (
                        ...
                        <><Upload className="w-10 h-10 text-gray-400" /><span className="text-sm text-gray-500 font-medium">Drag & drop or click to upload</span><span className="text-xs text-gray-400">Supports .csv, .xlsx, .xls</span></>
                    )}
                </div>
            ) : (
                <div className="flex-1 flex flex-col gap-3">
                    <div className="flex items-center gap-3 p-4 bg-brand-100/30 border border-brand-500/30 rounded-lg">
                        ...
                        <button onClick={handleRemove} className="text-gray-400 hover:text-red-500 transition-colors" title="Remove file"><X className="w-5 h-5" /></button>
                    </div>
