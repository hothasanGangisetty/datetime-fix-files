...
const SqlQueryTab = ({ connection, sqlState, setSqlState, onNext }) => {
    ...
    const handleExecute = async () => {
        ...
        try {
            ...
            log(`Query failed: ${err.response?.data?.message || err.message}`, 'error');
    ...
    return (
        <div className="h-full flex flex-col p-4 gap-4">
            ...
            <textarea className="flex-1 w-full p-3 border border-gray-300 rounded-lg font-mono text-sm bg-slate-50 focus:border-brand-700 focus:ring-1 focus:ring-brand-700 outline-none resize-none min-h-[120px]"
                ...
                placeholder="SELECT * FROM TableName" spellCheck={false} />
            <div className="flex justify-between items-center">
                <span className="text-xs text-gray-400">{sqlState.executed ? `${sqlState.columns.length} columns, ~${sqlState.count} rows` : 'Write a SELECT query and click Execute'}</span>
